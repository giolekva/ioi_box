...
./kubectl -n cms delete pod/register-users
...
while [[ $(kubectl -n cms get pods register-users -o 'jsonpath={..status.phase}') != "Succeeded" ]]; do
    echo "Waiting for pod"
    sleep 1
done

echo "Logs"
./kubectl -n cms logs register-users
